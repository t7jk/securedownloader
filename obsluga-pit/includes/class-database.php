...
class PIT_Database {
    ...
    public static function deactivate(): void {
        // Dane zostają zachowane po deaktywacji.
    }

    /**
     * Dodaje rekord pliku do bazy.
     *
     * @param array $data Dane pliku: first_name, last_name, pesel, tax_year, file_path, file_url
        ...
        return $wpdb->get_results(
            $wpdb->prepare(
                "SELECT f.*, 
                 ...
                 LEFT JOIN %i d ON f.id = d.file_id 
                 GROUP BY f.id 
                 ORDER BY f.uploaded_at DESC, f.last_name ASC, f.first_name ASC",
                self::$table_files,
                self::$table_downloads
            )
        );
            ...
            return $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT f.*, d.downloaded_at, d.ip_address
                     ...
                     LEFT JOIN %i d ON f.id = d.file_id
                     WHERE f.tax_year = %d
                     ORDER BY f.tax_year DESC, f.last_name ASC, f.first_name ASC",
                    self::$table_files,
                    self::$table_downloads,
                    $tax_year
                )
